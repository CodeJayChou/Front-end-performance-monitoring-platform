...
export function AlertsPage() {
  ...
  const submit = async (event: FormEvent) => {
    ...
    await mutate(() => client.createAlertRule(input));
  };
      ...
      <div className="content-grid alert-layout">
        <section className="panel">
          ...
          <form className="alert-form" onSubmit={submit}>
            ...
            <label>信号类型<select value={form.type} onChange={(event) => {
              const type = event.target.value as AlertRuleType;
              setForm({ ...form, type, name: type === "error_count" ? "错误数量过高" : "LCP P75 过高", threshold: type === "error_count" ? "10" : "2500" });
            }}><option value="error_count">错误数量</option><option value="performance_p75">性能 P75</option></select></label>
            {form.type === "performance_p75" ? <label>性能指标<select value={form.metric} onChange={(event) => setForm({ ...form, metric: event.target.value })}>{metrics.map((metric) => <option key={metric}>{metric}</option>)}</select></label> : null}
            ...
            <button className="primary-button wide" type="submit" disabled={saving}>{saving ? "正在保存…" : "创建规则"}</button>
          </form>
        </section>

        <section className="panel">
          <div className="panel-heading"><div><span>RULES</span><h2>当前规则</h2></div></div>
          {state.loading ? <LoadingState /> : state.error ? <ErrorState error={state.error} onRetry={refresh} /> : state.data?.rules.length ? (
            <div className="alert-rule-list">{state.data.rules.map((rule) => (
              <article className="alert-rule" key={rule.id}>
                ...
                <div className="alert-actions"><button type="button" disabled={saving} onClick={() => void mutate(() => client!.setAlertRuleEnabled(rule.id, !rule.enabled))}>{rule.enabled ? "停用" : "启用"}</button><button className="danger-action" type="button" disabled={saving} onClick={() => {
                  if (window.confirm(`确定删除规则“${rule.name}”及其告警历史吗？`)) void mutate(() => client!.deleteAlertRule(rule.id));
                }}>删除</button></div>
              </article>
            ))}</div>
