...
import { EmptyState, ErrorState, formatDate, LoadingState, PageHeader } from "../components/Ui";
      ...
      <div className="content-grid source-map-layout">
        <section className="panel">
          ...
          <form className="source-map-form" onSubmit={submit}>
            ...
            <button className="primary-button" type="submit" disabled={busy || !sourceMap}>{busy ? "正在上传…" : "上传并启用"}</button>
          </form>
        </section>
        <section className="panel">
          <div className="panel-heading"><div><span>ARTIFACTS</span><h2>已上传映射</h2></div></div>
          {state.loading ? <LoadingState /> : state.error ? <ErrorState error={state.error} onRetry={refresh} /> : state.data?.length ? <div className="source-map-list">{state.data.map((item) => <article key={item.id}><div><strong>{item.artifactName}</strong><span>{item.release}{item.dist ? ` · ${item.dist}` : ""}</span></div><div><span>{item.sourceCount} 个源文件</span><span>更新于 {formatDate(item.updatedAt)}</span></div><code>{item.contentHash.slice(0, 16)}</code><button type="button" disabled={busy} onClick={() => void remove(item.id, item.artifactName)}>删除</button></article>)}</div> : <EmptyState title="还没有 Source Map" description="上传后，新处理的同 Release 错误会自动尝试源码还原。" />}
        </section>
      </div>
    </div>
  );
